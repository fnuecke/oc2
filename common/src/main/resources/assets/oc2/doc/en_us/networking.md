...
`setup-network.lua` starts the DHCP server when you configure a computer as one. It does not survive a reboot. See the "Daemons" section below for how to start it again, or how to make it start on its own.
...
Note that `scp` and `ssh` are the client tools. Connecting to a computer *from* elsewhere needs a server running on it. See the "Daemons" section below.

## The Internet
For access outside the game, a network needs an [internet gateway](block/internet_gateway.md) on it. Computers then use their ordinary [network interface cards](item/network_interface_card.md) and route through it.

The gateway offers no DHCP and no name server, so set an address, a route and a name server yourself. See the [internet gateway](block/internet_gateway.md) entry for an example.

## Daemons
The network daemons are shipped but not started at boot. Start the one you need by hand:
...
The `ssh` server refuses to let anyone in while an account has a blank password, which is what `root` ships with. Set one with `passwd` before connecting for the first time. The `telnet` server has no such restriction, and accepts the blank password as-is, so anyone who can connect to the computer can log into it.

None of these survive a reboot. To start one at boot, give its script a boot-sequence prefix:  
