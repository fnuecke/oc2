...
Robôs são, essencialmente, [computadores](../block/computer.md) móveis. Devido à sua natureza não estacionária, existe algum comportamento que difere daquele de um computador normal. Eles não podem conectar [interfaces BUS](../block/bus_interface.md), por exemplo. Em vez de dispositivos de cartão, eles suportam dispositivos de módulo. Os mesmos são dispositivos especializados tendo em conta a mobilidade dos robôs.
