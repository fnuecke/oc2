...
`find(typeName:string):Device` возвращает обертку над устройством по его типу (имени). Если устройств с таким названием несколько, будет передан случайный. В качестве типа можно указать название подключенной к нему [интерфейсной шины](block/bus_interface.md).
